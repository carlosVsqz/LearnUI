...
public interface DeliverableRepository extends JpaRepository<Deliverable, UUID> {
    ...
    @Query("SELECT new com.starterkit.springboot.brs.dto.DeliverableResourcesDTO(d.id, d.dueDate, d.title, d.description,ta.status ) FROM Deliverable d JOIN TeckerAssignment ta ON d.id = ta.deliverableId WHERE d.id = :deliverableId")
